...
#### 为什么 `vpc_cidr` 必须保持 `/16`

`cidrsubnet(prefix, newbits, netnum)` 的语义：

- **prefix**：父网段（这里是 `var.vpc_cidr`）
- **newbits**：在父掩码后再加几位给子网用 → 子网掩码 = 父掩码 + newbits
- **netnum**：取第几个子网（从 0 开始）

代码里写死了 `newbits = 8`，所以"子网掩码 = `vpc_cidr` 掩码 + 8"。一旦 `vpc_cidr` 不是 `/16`，下游全塌：

| `vpc_cidr` 输入 | 子网掩码 | 单子网 IP 数 | 结果 |
|---|---|---|---|
| `10.2.0.0/16`（正确） | `/24` | 256 | ✅ Glue/Lambda ENI 够用 |
| `10.2.0.0/20` | `/28` | 16（AWS 保留 5 个 → 可用 11） | ⚠️ 能 apply，但 ENI 一多就耗尽 |
| `10.2.0.0/24` | `/32` | 1 | ❌ AWS 拒绝（subnet 最小 `/28`），apply 报错 `InvalidSubnet.Range` |
| `10.2.0.0/28` | `/36` | — | ❌ `cidrsubnet` 直接报错（掩码不能 >32） |

具体到失败例子，假设传入 `vpc_cidr = "10.2.0.0/24"`：

```
cidrsubnet("10.2.0.0/24", 8, 0)    → 10.2.0.0/32     ← 单 IP，AWS 拒绝
cidrsubnet("10.2.0.0/24", 8, 100)  → 10.2.0.100/32   ← 同上
```

**结论**：要么保持 `/16` 不动，要么连 [main.tf:18,30](terraform/modules/networking/main.tf#L18) 的 `cidrsubnet` 第二个参数也一起调，两边联动改。只改一边必坑。
...
## Snowpipe SQS 三个参数的含义与取值合理性

[terraform/modules/snowpipe/main.tf:65-72](terraform/modules/snowpipe/main.tf#L65-L72) 创建 Snowpipe 用的 SQS queue：
...
三个参数都不是装饰性的，每一个都对应一类故障场景。下面拆开讲。

### 1. `visibility_timeout_seconds = 300`

#### 含义

消费者（Snowpipe）从队列里 `ReceiveMessage` **拿到一条消息后**，这条消息对其他消费者**隐身** 300 秒。Snowpipe 必须在 300s 内处理完并 `DeleteMessage` 确认成功，否则消息会重新出现在队列里被重投。

这是 SQS 的 **at-least-once delivery（至少一次投递）** 机制——SQS 不知道消费者是真正"处理完了"还是"中途崩溃了"，它只看消费者有没有在隐身期内 Delete。

#### 举例（一个 Silver Parquet 文件触发 ingest）

```
T=0    S3 PUT → SNS → SQS 收到消息（visibility timer 还没启动）
T=10   Snowpipe 调 ReceiveMessage 拿到消息 → 消息隐身到 T=310
T=10   Snowpipe 触发 COPY INTO IODP_DC.SILVER.DC_WIDE
T=40   COPY 完成 → DeleteMessage → 消息彻底消失 ✓

如果 Snowpipe 在 T=20 挂了（COPY 没完成、没 Delete）：
T=310  消息重新可见 → 下一次轮询重新拉到 → 重新 COPY
       (幂等：Snowflake 内部按 file 名去重，不会重复加载)
```

#### 为什么是 300s 而不是 30s

COPY INTO 大文件可能要几十秒，再加上 Snowflake 端排队 / 网络抖动的余量。Snowflake 官方推荐 Snowpipe SQS visibility **至少 300s**——少于这个值会出现"消费者还没处理完，消息就又被另一个消费者拉走"的双重处理问题。

### 2. `receive_wait_time_seconds = 10`（long polling）

#### 含义

消费者调 `ReceiveMessage` 时，如果队列是空的，AWS 会把这个连接**挂起最多 10s**等新消息进来；期间一旦有消息到达就立刻返回。`= 0` 是 short polling（空就立即返回）。
...
short polling (=0):
  ...
  T=0.1  poll → 空 → 立即返回
  T=0.2  poll → 空 → 立即返回
  …      1 分钟内几百次空调用 → 浪费 SQS API 费用 + 增加延迟
  T=3.0  消息到达，但要等下一次 poll 才能拿到

long polling (=10):
  T=0    poll → 连接挂起
  T=3    消息到达 → 连接立即返回这条消息  ✓ (3s 拿到)
  T=13   poll → 挂起
  T=23   挂满 10s 没消息 → 返回空
```
...
- **空闲时 API 调用从几百次/分钟降到 6 次/分钟**——直接降 SQS 费用；
- **新消息几乎瞬时拿到**（不像 short polling 要等下一轮 poll）；
- Snowpipe SDK 默认就是 long polling，跟队列设置一致。

`= 10` 是 SQS 允许的最大值（区间 0–20，但 Snowpipe 内部用 10）。

### 3. `message_retention_seconds = 86400`（1 天）够用吗？

#### 含义

消息在队列里**没被消费的最大寿命**，到期 SQS 自动丢弃。**注意它不是"消费一次的超时"**——那是 visibility timeout 的事。

#### 正常 vs 异常场景

正常情况下 Snowpipe 消费一条消息是**秒到分钟级**的，1 天的预算用不上 0.1%。retention 这个参数真正在防御的是**消费方长时间不可用**的场景：

| 场景 | 1 天够吗 |
|---|---|
| 单个文件正常 ingest（< 1 分钟） | ✅ 远远够 |
| Snowpipe 偶发故障，几小时恢复 | ✅ 够 |
| Snowflake 整个区域宕机 > 24h | ❌ 消息丢失，需要手动恢复 |
| Snowpipe 配额 / 角色被错误改动几天没人发现 | ❌ 同上 |

#### 兜底机制：`ALTER PIPE … REFRESH`

如果消息真的因为消费方挂太久被 SQS 丢了，Snowpipe 不是**完全没救**——可以手动跑：

```sql
ALTER PIPE PIPE_DC_WIDE REFRESH;
```

它会**主动扫一遍 stage 指向的 S3 路径**，把所有"还没在 Snowflake 内部加载历史里"的文件**重新触发**一次 COPY。这是 Snowpipe 设计上自带的"我没收到事件没关系，我自己去看 S3 有什么"的兜底——哪怕 SQS 消息全部丢了也能补回来。

#### 为这个项目选 1 天的判断

- Silver 是**每天一次**写入（每天 UTC 10:00 Glue Job），单天数据量小；
- Snowflake SLA 高，区域级宕机 >24h 极罕见；
- 最坏情况靠 `ALTER PIPE REFRESH` 兜底——这不是"我们没考虑"，而是 Snowpipe 自带的 recovery path；
- DLQ 加复杂度，对每天 1 个文件的吞吐量不划算。

**结论**：1 天合理，是个 trade-off 不是 bug。

#### 如果想更稳的两个加固方向（不强求）

1. **把 retention 提到 4–14 天**（SQS 上限 14 天）——代价几乎为零，多一份保险；
2. **加 DLQ + redrive policy**——记录"反复失败的消息"以便排查 schema 漂移之类问题。

什么情况下值得加？吞吐量上来（每天几百个文件）后，如果有"少量文件因 schema 不匹配持续失败"的可能，DLQ 能帮你单独捞出来排查；现在每天 1 个文件，肉眼就能发现，加 DLQ 是 over-engineering。

### 三个参数串起来看

```
        ┌─────────────────────── 1 天 ──────────────────────┐
        │  retention：消息进队列后没人消费的最大寿命           │
        │                                                     │
        │   ┌────── 10s ──────┐                              │
        │   │ wait time:      │                              │
        │   │ poller 等新消息 │                              │
...
三个参数对应三种不同的"时间维度"，**不是同一个东西的不同值**——这是新人最容易搞混的点。

---

## Snowpipe SQS 失败可观测性的 gap 与 DLQ 加固

紧接上一节——visibility timeout 的"重投"机制有个自然延伸问题：**重试有没有上限？上限到了之后失败有没有被记录用于 debug？** 这一节回答这两点，并讲清楚 [terraform/modules/snowpipe/main.tf](terraform/modules/snowpipe/main.tf) 加上的 DLQ 设计的边界。

### 第一层：原版（无 DLQ）的行为

[terraform/modules/snowpipe/main.tf](terraform/modules/snowpipe/main.tf) 最初写的 SQS 队列**没有 `redrive_policy`、没有 DLQ**。这种情况下：

```
T=10    Snowpipe 拉消息 → 处理失败 → 不 Delete
T=310   visibility 过期 → 消息重新可见 → ApproximateReceiveCount=2
T=620   再失败 → ApproximateReceiveCount=3
T=930   再失败 → ApproximateReceiveCount=4
…
T=86410 (1 天后) retention 到期 → SQS 静默丢弃消息  ❌
```

`ApproximateReceiveCount` 这个计数器 SQS 内部一直在涨，但**没有 DLQ 的话它涨到天上去也没人管**——唯一的终止条件就是 retention 过期。

### 关键事实：Snowpipe 不靠 SQS 重投兜 COPY 失败

之前那个时序图（"如果 Snowpipe 在 T=20 挂了 → T=310 消息重新可见 → 重新 COPY"）里隐含了一个**容易被读者过度泛化**的假设：好像每次 COPY 失败 SQS 都会重投。**实际不是这样**。

Snowpipe AUTO_INGEST 的工作流：

1. 拉 SQS 消息，看 S3 路径
2. 提交 COPY INTO，结果（成功 / 失败 / 部分加载）记入 Snowflake 的 `COPY_HISTORY`
3. **不管 COPY 成败**，Snowpipe 都会 `DeleteMessage` 把这条 SQS 消息从队列里删掉

也就是说：

- 如果 **Snowpipe 进程正常但 COPY 失败**（schema 不对、Parquet 损坏、文件路径不匹配 stage）→ 消息从 SQS 消失，错误**只在 `COPY_HISTORY` 里有记录**，SQS 不会重投
- 只有 **Snowpipe 自己物理"挂了"或者 visibility 期内没来得及 ack** 的场景，消息才会回到队列被重投

之前的时序图描述的是后一种（消费方挂了），不是前一种（COPY 失败）。这点要分清。

### 第二层：失败被记录在哪里

**Snowflake 端**（这是 COPY 失败的**唯一**真实记录处）：

| 查询 | 看什么 |
|---|---|
| `SELECT SYSTEM$PIPE_STATUS('PIPE_DC_WIDE')` | 实时看 pipe 状态：`lastReceivedMessageTimestamp` / `lastErrorMessage` / `lastErrorRecordTimestamp` |
| `SELECT * FROM TABLE(INFORMATION_SCHEMA.COPY_HISTORY(TABLE_NAME=>'IODP_DC_DEV.SILVER.DC_WIDE', START_TIME=>...))` | 每次 COPY 一行：`STATUS`（Loaded/Load failed/Partially loaded）+ `FIRST_ERROR_MESSAGE` + `FILE_NAME` |
| `SELECT * FROM SNOWFLAKE.ACCOUNT_USAGE.COPY_HISTORY` | 历史 90 天，更全（但有 ~45min 延迟） |

**AWS 端**只有 CloudWatch SQS 指标，**指标不告诉你为什么失败**：
- `ApproximateNumberOfMessagesVisible`：积压量
- `ApproximateAgeOfOldestMessage`：最老消息已经在队列里多久——是**最有价值的间接信号**

**项目原有的兜底**：[snowflake_sql/08_freshness_alert.sql](snowflake_sql/08_freshness_alert.sql) —— 检查 SILVER.DC_WIDE 里 `dt` 是不是按预期推进。如果数据没进表，第二天会发现，但**慢且粗**：只告诉你"数据没到"，不告诉你"哪个文件、什么原因"。

### 第三层：DLQ 改造的设计边界

加 DLQ 是性价比很高的加固，但**它能 cover 的是哪一层失败必须先讲清楚**——否则会被错误期待。

#### DLQ 实际能 cover 的失败类型

| 失败场景 | 会进 DLQ 吗 | 为什么 |
|---|---|---|
| Schema 不匹配、Parquet 损坏（COPY 失败） | ❌ 不会 | Snowpipe 删消息后才记 COPY_HISTORY |
| Snowpipe 进程在处理中途崩溃 | ✅ 会 | 没 Delete → 重新可见 → receive_count 累加 → maxReceiveCount=5 后转 DLQ |
| Snowpipe 端响应慢，每次 visibility 都过期 | ✅ 会 | 同上 |
| Snowpipe 完全不轮询（IAM 撤销、pipe 被禁、Snowflake 区域故障） | ❌ 不会 | receive_count 永远是 0，redrive 不触发 |

最后一行特别重要——**DLQ 漏掉了"完全不轮询"的场景**。所以单靠 DLQ 还不够，必须配一个**主队列消息陈旧告警**（`ApproximateAgeOfOldestMessage`）来兜底"消息躺着没人理"的情况。

#### 落地的 TF 改动

[terraform/modules/snowpipe/main.tf](terraform/modules/snowpipe/main.tf) 加了 4 个资源：

```hcl
# 1. DLQ — 14 天 retention 给排查留时间
resource "aws_sqs_queue" "snowpipe_dlq" {
  name                      = "iodp-dc-snowpipe-dlq-${var.environment}"
  message_retention_seconds = 1209600
}

# 2. 主队列加 redrive_policy
resource "aws_sqs_queue" "snowpipe" {
  # ... 原有 visibility/retention/wait_time ...
...
# 3. Alarm A：DLQ 一有消息立即告警（1 个评估周期 5min）
...
# 4. Alarm B：主队列最老消息 >30min 持续 10min（兜"完全不轮询"）
...
[terraform/modules/observability/main.tf](terraform/modules/observability/main.tf) 的 `aws_sns_topic_policy.alerts_allow_events` 也补了一条 `cloudwatch.amazonaws.com` 的 publish 允许——**这是必须的**，CloudWatch alarm 发到 SNS topic 跨服务边界，没这条 policy 告警会静默丢失。

#### MTTD 改善

| 失败类型 | 改造前 MTTD | 改造后 MTTD | 责任告警 |
|---|---|---|---|
| Snowpipe 处理慢 / 卡住 | ≥24h | ~25min | DLQ alarm（新增） |
| Snowpipe 完全不轮询 | ≥24h | ~30min | queue-age alarm（新增） |
| COPY 失败（schema 漂移等） | ~24h | ~24h | freshness alert（不变，**这就是合适的解**） |

第三行刻意不动——这个项目每天 1 个文件，freshness alert 第二天发现"昨天的 dt 没进表"就够用了。**不需要再起一个并行的 Snowflake task 去扫 `COPY_HISTORY` 找 `STATUS != 'Loaded'`**：多一个对象要维护、要授权、要监控，换来的 MTTD 改善对这个 scale 没意义。当吞吐量上到每天几十/几百个文件、单文件失败的代价（数据空洞）显著变高时再考虑，不是现在。

DLQ 改造的定位因此是**对 freshness alert 的补充**——它把原本 freshness alert 也覆盖不到的两类故障（处理慢、不轮询）压到分钟级；COPY 失败这条线 freshness alert 已经是合适的兜底，**别为了 100% 对称去填一个不需要的洞**。

### 第四层：Redrive — 把 DLQ 消息打回主队列

#### AWS 原生支持吗？

**支持，2023 年加的 [`StartMessageMoveTask`](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_StartMessageMoveTask.html) API**。在这之前你得自己写循环 `receive → send → delete` 的脚本（容易丢消息、不好暂停）。现在 SQS 服务端托管整个搬运过程，你只发一条命令。

#### 命令

```bash
# 把 DLQ 里所有消息移回原始来源队列（默认行为）
aws sqs start-message-move-task \
  --source-arn "arn:aws:sqs:us-east-1:123456789012:iodp-dc-snowpipe-dlq-dev" \
  --region us-east-1
```

关键点：
- `--destination-arn` 可省略——默认就是"通过 redrive_policy 把消息送进 DLQ 的那个原始队列"。这正好是我们要的（DLQ → 主队列）
- 可以用 `--max-number-of-messages-per-second` 限速，避免突然把几千条消息打回去把 Snowpipe 冲挂
- 任务在 SQS 服务端异步跑，命令瞬间返回一个 `TaskHandle`
- 用 `aws sqs list-message-move-tasks` 查进度，`aws sqs cancel-message-move-task` 中途取消

#### 是否需要 Make 子命令？

需要，理由有三：
1. **DLQ 的 ARN/URL 来自 `terraform output`**，每次手动拼太啰嗦也容易拼错
2. **redrive 是个"按下后会真实改变状态"的操作**，封装到 make 里能在脚本里加 echo 提示和 ENV 检查，避免操作错环境
3. **排查时是"一组动作"**：先 `status` 看积压、再 `peek` 看消息内容、修底层问题、最后 `redrive`——封装成命名一致的 5 个子命令对操作者友好

[Makefile:139-180](Makefile#L139-L180) 加了一组 `snowpipe-dlq-*`：

| 命令 | 用途 |
|---|---|
| `make snowpipe-dlq-status` | 看 DLQ 里有多少消息、最老多久 |
| `make snowpipe-dlq-peek` | 拿一条消息看内容（visibility=1s，等于"瞄一眼"） |
| `make snowpipe-dlq-redrive` | 调 `StartMessageMoveTask`，全部打回主队列 |
| `make snowpipe-dlq-redrive-status` | 看在跑的 / 历史 redrive task 进度 |
| `make snowpipe-dlq-redrive-cancel HANDLE=...` | 中途取消 redrive（很少用，预案） |
...
```
1. make snowpipe-dlq-status ENV=prod
   → 看到积压 12 条，最老 47 min

2. make snowpipe-dlq-peek ENV=prod
   → 看一条消息，发现都是同一个 S3 路径模式

3. 去 Snowflake 查 COPY_HISTORY → 找到 Snowpipe 的具体 lastErrorMessage
   → 比如 "Storage Integration permission denied"
   → 进而发现 Snowflake 那边角色被改了

4. 修底层：恢复角色权限

5. make snowpipe-dlq-redrive ENV=prod
   → AWS 后台慢慢把 12 条搬回主队列

6. make snowpipe-dlq-redrive-status ENV=prod
   → 几分钟后看到 ApproximateNumberOfMessagesMoved = 12

7. 等 freshness alert 第二天确认数据回来正常
```

第 3 步那个"去 Snowflake 查 COPY_HISTORY"是排障时的常规动作——**DLQ alarm 告诉你"出事了"，COPY_HISTORY 告诉你"具体哪个文件、为什么"**。两边职责清楚：alarm 负责发现，`COPY_HISTORY` 负责诊断。

### 一句话总结这次改造

加 DLQ + 两个 alarm 把"Snowpipe 端处理失败"和"Snowpipe 不轮询"两类故障的 MTTD 从 24h 压到 25–30min；**COPY 失败（schema 漂移之类）继续由 freshness alert 兜底，刻意不补 Snowflake task——对每天 1 文件的吞吐量这是 over-engineering**。所有运维路径用 5 个 `snowpipe-dlq-*` make 子命令封装，AWS-native 的 `StartMessageMoveTask` 让 redrive 不需要写循环脚本。

---

## S3 Lifecycle filter 的 prefix 究竟匹配什么

[terraform/modules/storage/main.tf:66-77](terraform/modules/storage/main.tf#L66-L77) 给 Bronze 桶配了一条 DLQ 30 天清理规则：
...
rule {
  ...
  filter {
    prefix = var.dead_letter_prefix   # 默认值 = "dead_letter/"
  }
...
第一次看可能会想：**这个 `prefix` 是不是要写完整路径，比如 `s3://iodp-dc-bronze-dev/dead_letter/`**？答案：不需要、也不应该写。下面把机制讲清楚。

### 关键事实：S3 内部没有"路径"概念

S3 的存储模型是**扁平的 key-value 桶**——每个对象只有两个标识维度：

```
s3://iodp-dc-bronze-dev-123456789012/dead_letter/failed_at=2026-04-27/download_channel/narrow/dt=2025-12-15/store=ios/part-0.parquet
└─ scheme ─┘└────────── bucket ──────────┘└──────────────────────────── Key ────────────────────────────┘
```

- **scheme** (`s3://`) ：客户端工具用来识别这是 S3 协议，**S3 服务端从来不存这个字符**
- **bucket** (`iodp-dc-bronze-dev-...`) ：通过 `aws_s3_bucket_lifecycle_configuration` 的 `bucket = ...` 参数指定，filter 是这个桶**内部**的二级筛选
- **Key**：从桶名后第一个 `/` 开始的整段字符串，**包含所有的 `/`**，但 S3 不把它解释成"路径分隔符"——它只是 key 字符串里的一个普通字符

所谓的"文件夹"完全是人为约定：把 key 里的 `/` 当成路径分隔显示出来而已。S3 服务端只看 key 字符串本身。

### prefix 是什么匹配模式

[aws_s3_bucket_lifecycle_configuration](https://docs.aws.amazon.com/AmazonS3/latest/userguide/intro-lifecycle-rules.html) 的 `filter.prefix` 是**对 Object Key 做纯字符串前缀匹配**：

- ❌ 不是 glob（不支持 `*` `?`）
- ❌ 不是 regex
- ❌ 不分层级（不是"只匹配第一层"）
- ✅ 就是 `key.startswith(prefix)`

所以 `"dead_letter/"` 一个字符串就把**任意深度的子路径**全部覆盖，包括分区路径里出现 `=` 这种特殊字符也没事——S3 不会特殊解释。

### 具体匹配示例

DLQ 实际存在的几种 key 形态（来自 [glue/dlq_replay.py:19-20](glue/dlq_replay.py#L19-L20) 和本文档前面 DLQ 设计章节）：

| Object Key | 是否匹配 `prefix = "dead_letter/"` |
|---|---|
| `dead_letter/failed_at=2026-04-27/download_channel/narrow/dt=2025-12-15/store=ios/part-0.parquet` | ✅ |
| `dead_letter/failed_at=2026-04-27/download_channel/narrow/dt=2025-12-15/store=ios/part-0.parquet.error.json` | ✅ |
| `dead_letter/failed_at=2026-05-02/dq_failure_dt=2026-04-30/part-0001.parquet` | ✅ |
| `download_channel/narrow/dt=2026-04-25/...` （正常 Bronze 文件） | ❌（不会被 30 天 expiration 误删） |
| `athena-results/...` | ❌ |

效果：所有 DLQ 文件 30 天后被清理，正常 Bronze 数据走的是另一条 `bronze-standard-lifecycle` rule（IA → Glacier IR → 365 天 expire）——**两条 rule 作用域不重叠**，靠 prefix 隔离开。

### 容易被忽略的细节：尾部那个 `/` 不能省

`"dead_letter/"`（带斜杠）vs `"dead_letter"`（不带斜杠）：

| 假想存在的 Key | `"dead_letter/"` 匹配 | `"dead_letter"` 匹配 |
|---|---|---|
| `dead_letter/foo.parquet` | ✅ | ✅ |
| `dead_letter_archive/bar.parquet` | ❌ | ✅ ⚠️ |
| `dead_letterfoo.parquet` | ❌ | ✅ ⚠️ |

如果哪天有人在桶里建了一个 `dead_letter_archive/` 之类的"邻居"前缀（比如归档冷存档），**带斜杠的写法不会误伤**——后两个 key 不会被 30 天 lifecycle 砍掉。这是 S3 prefix 配置的标准防御写法。

带斜杠还是不带斜杠是**纯字符串问题**，S3 不会"自动加上斜杠让你不踩坑"——必须自己写对。

### 仓库内一致性交叉验证

整个仓库各处对 DLQ 前缀的处理用的都是**相对 Key + 带尾斜杠**，没人在 API 调用里写 `s3://...` 整个 URI：

| 位置 | 用法 |
|---|---|
| [terraform/modules/storage/main.tf:71](terraform/modules/storage/main.tf#L71) | `prefix = var.dead_letter_prefix`（= `"dead_letter/"`，给 lifecycle filter） |
| [lambda/dlq_weekly_report/handler.py:33](lambda/dlq_weekly_report/handler.py#L33) | `prefix = "dead_letter/"`（给 boto3 list_objects_v2 paginator） |
| [glue/dlq_replay.py:63](glue/dlq_replay.py#L63) | `dlq_prefix = "dead_letter/failed_at=PLACEHOLDER/"`（替换 PLACEHOLDER 后给 boto3） |
| [Makefile:143](Makefile#L143) | `aws s3 ls "s3://...-bronze-.../dead_letter/" --recursive`（注意：CLI 工具接受 `s3://` URI，但传给 boto3/Terraform 的 prefix 参数永远是相对 key） |

最后一行那个区分很重要——**CLI 工具（`aws s3 ls`、`aws s3 cp`）接受 `s3://bucket/prefix` 这种 URI** 是为了人类好用，但底层 SDK 调用（`list_objects_v2(Bucket=..., Prefix=...)`、Terraform `filter.prefix`）的 `Prefix` 参数永远只是 key 的相对前缀。两套接口别搞混。

### 一句话总结

S3 prefix 永远是**桶内 Object Key 的字符串前缀**，不带 `s3://`、不带桶名、不分层级、不是 glob。`dead_letter/` 一个前缀递归覆盖 `failed_at=.../dq_failure_dt=.../...parquet` 所有变体；带尾斜杠是有意识的边界保护，删掉会把 `dead_letter_archive/` 这类邻居误伤——别动。

---

## 56. Snowpipe IAM 信任关系：为什么需要 ExternalId

### 起因

看 [terraform/modules/snowpipe/main.tf:20-33](terraform/modules/snowpipe/main.tf#L20-L33) 这段 `assume_role_policy`：
...
两个问题：
1. 为什么创建 `sts:AssumeRole` 时需要 `ExternalId`？是因为 Snowflake 是外部对象吗？
2. Snowpipe 是 Snowflake 中创建的外部对象吗？

### 1. 为什么需要 `ExternalId`：防"混淆代理人攻击"

是的，**正是因为 Snowflake 是另一个 AWS 账户里的外部主体**，所以需要 ExternalId。

#### 信任链结构

```
Snowflake AWS Account（Snowflake 公司账户）
    └── IAM User: var.snowflake_iam_user_arn
            ↓ AssumeRole（跨账户）
Your AWS Account（你的账户）
    └── IAM Role: iodp-dc-snowpipe-s3-{env}
            └── 有读 Silver bucket 的权限
```

#### ExternalId 解决什么问题

Snowflake 是 **多租户 SaaS** —— 它给所有客户用的那个 IAM User ARN 是 *共用的*。

假如没有 ExternalId：
- 攻击者也是 Snowflake 客户 → 他天然知道 Snowflake 的 IAM User ARN
- 如果他能拿到你的 Role ARN → 他就能让 Snowflake 帮他 AssumeRole 进你的账户读你的数据

**ExternalId 是 Snowflake 给你的 account 单独分配的"暗号"**：
- 只有你的 Snowflake account 持有这个 ExternalId
- 即使攻击者拿到你的 Role ARN，他自己 Snowflake account 上的 ExternalId 是另一个值，对不上 → AWS 拒绝 AssumeRole

这是 AWS 官方推荐的 **"third-party access"** 模式：[AWS docs - How to use external ID](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html)。

口诀：**"跨账户 + 多租户 SaaS" = ExternalId 必备**。

### 2. Snowpipe 是 Snowflake 的对象吗：是，但这个 Role 服务的是 Storage Integration

Snowpipe 本身确实是 Snowflake 里的 `PIPE` 对象，但这个 IAM Role 服务的不只是 Pipe，而是 **Storage Integration** 这一整套机制。

#### Snowflake 侧对象层级（创建顺序）

```
1. STORAGE INTEGRATION   ← 它持有 var.snowflake_iam_user_arn 和 ExternalId
        ↓ 引用
2. STAGE (external)      ← 指向你的 S3 bucket
        ↓ 引用
3. PIPE                  ← AUTO_INGEST，监听 SQS
```

[main.tf:8-11](terraform/modules/snowpipe/main.tf#L8-L11) 文件头注释也印证了这点：

> Snowflake-side resources (database, schemas, roles, grants, **storage integration**) live in terraform/modules/snowflake/. The Pipe/Stage/FileFormat objects are created via snowflake_sql/04_pipe.sql

所以这个 IAM Role 是给 **Storage Integration** 用的 —— Pipe 通过 Stage 间接复用同一套凭证去读 S3，这就是 Snowflake 推荐的"凭证集中管理"模式（不让 Stage 各自持有 AWS keys）。

### 3. 为什么注释里写"bidirectional"

[main.tf:4](terraform/modules/snowpipe/main.tf#L4) 注释里说 "IAM role with Snowflake trust (bidirectional)"，意思是：

| 方向 | 谁信任谁 | 体现在哪 |
|---|---|---|
| AWS → Snowflake | 允许 Snowflake 的 IAM User AssumeRole 进来 | 这段 `assume_role_policy` |
| Snowflake → AWS | Storage Integration 里 `STORAGE_AWS_ROLE_ARN` 指向这个 Role | `terraform/modules/snowflake/` |

#### 先有鸡还是先有蛋的循环

- 创建 Storage Integration 时，Snowflake 才会生成 `STORAGE_AWS_IAM_USER_ARN` 和 `STORAGE_AWS_EXTERNAL_ID`（每个 Snowflake account 不同）
- 但你创建 IAM Role 时就需要这两个值

实际部署顺序通常是：
1. 先用占位符在 Snowflake 创建 Storage Integration
2. `DESC INTEGRATION <name>` 拿到 `STORAGE_AWS_IAM_USER_ARN` 和 `STORAGE_AWS_EXTERNAL_ID`
3. 把这两个值回填到 Terraform variables（`snowflake_iam_user_arn`、`snowflake_external_id`）
4. `terraform apply` 创建/更新 IAM Role
5. 再 `DESC INTEGRATION` 验证 Snowflake 那边能成功 AssumeRole

### 一句话总结

ExternalId 是 **Snowflake 多租户 SaaS 跨账户访问 S3 时防"混淆代理人攻击"的凭证**；这个 IAM Role 的真正服务对象是 Snowflake 里的 **Storage Integration**（Pipe 通过 Stage 间接使用），所以"Snowpipe 是 Snowflake 的外部对象"这个判断方向是对的，只是更准确地说是 **Storage Integration → Stage → Pipe** 这一整条链都在 Snowflake 侧。

---

## Dropzone 桶授权的"鸡生蛋"——为什么 demo 不存在，cross-account 才存在

README 的 Initial Setup #3 让你手动 `aws s3 mb` 建 dropzone 桶，下一步 `make init` 跑 `terraform apply` 创建 bronze IAM role。**直觉会问**：

> 光创建 dropzone 桶没用啊，还要授权给 bronze job 访问；但是你建桶的那一刻 bronze role 还不存在，怎么把权限挂上去？这不是鸡生蛋蛋生鸡吗？

直觉看起来很合理，但**当前 single-account demo 设计下这个鸡生蛋根本不存在**。误判来源是把"授权长在哪一边"想错了。下面拆开讲。

### 误判 1：授权不长在 dropzone 桶上，长在 bronze role 上

直觉的 mental model：
> 给 bronze 授权读 dropzone = 改 dropzone 的 **bucket policy** 加 bronze 的 ARN

但代码不是这么做的。看 [terraform/modules/glue_etl/main.tf:53-59](terraform/modules/glue_etl/main.tf#L53-L59)（节选）：

```hcl
resource "aws_iam_role_policy" "glue_dropzone_read" {
  ...
  policy = jsonencode({
    Statement = [{
      Effect   = "Allow"
...
授权"长在 bronze role 这一边"——给 bronze 自己的 IAM role 一个**主动权限**（"我可以读那个桶"），而不是改 dropzone 桶的 policy（"那个桶允许 bronze 来读"）。

**Same-account 场景下，IAM role policy 单边授权就够了**——桶不需要任何 policy（除非要拒绝其他人，那是另一回事）。`s3:GetObject` 调用时 S3 服务端会按以下顺序判断：caller 是不是同一 account → 同账户的 IAM policy 是不是允许 → 没有 explicit deny → 通过。整个过程**不查桶上的 policy**。

### 误判 2：以为 IAM 创建 policy 时会校验 ARN 对应的资源存在

ARN 是**字符串**。`aws_iam_role_policy` 创建的瞬间 IAM 服务**不会**反查 S3 看 `dataai-dropzone-dev-xxx` 是不是真的存在，它就老老实实把字符串存下来。

存在性校验**只发生在 bronze 真正调 S3 API 的运行时**。所以"先建 IAM policy 引用 dropzone"和"先建 dropzone 桶本身"**没有顺序约束**。

这也解释了为什么 [terraform/main.tf:91](terraform/main.tf#L91) 大胆用字符串拼接 ARN（`"arn:aws:s3:::${var.dropzone_bucket_name}"`）而不是 `data "aws_s3_bucket"` lookup——它不需要桶在 plan 阶段存在。

### 完整时序（证明没有鸡蛋问题）

```
Step 1 (Initial Setup #3):  aws s3 mb s3://dataai-dropzone-dev-XXX
   → S3 桶存在了

Step 2 (make init):  terraform apply
   → 创建 bronze IAM role
   → 给 role 挂 policy（policy 里写 "arn:aws:s3:::dataai-dropzone-dev-XXX"）
   → IAM 不校验 ARN 存在性，policy 顺利创建 ✓

Step 3 (make run-etl):  Glue 启动 bronze
   → bronze 用它的 role 去 S3 GetObject
   → S3 查 role 的 policy → ARN 匹配 → caller 是 Glue 自己 → 允许
   → 读到数据 ✓
```

**反过来做也行**：先 `terraform apply` 再 `aws s3 mb`，中间那段时间 bronze role 已经存在并且 policy 已经写了 ARN，但桶还没建——这种状态下：
- `terraform apply` 不会失败（IAM 不校验）
- bronze 此时如果跑会失败（S3 返回 NoSuchBucket）
- 等你建好桶之后，bronze 就能跑

桶存在与否只在 bronze 实际**运行时** matter，跟 IAM 创建时序无关。

### 但 production cross-account 场景下，鸡生蛋是真的

如果生产里 dropzone 是 **Data.ai 拥有的桶**（在他们的 AWS account 里），场景就变了：

- Data.ai 桶**必须**有一条 bucket policy explicit 允许你的 bronze role ARN（cross-account 必须双边都同意，单边 IAM 不够）
- 你的 bronze role ARN 在 `terraform apply` 之前**不存在**
- Data.ai 想把 ARN 写进他们的 bucket policy 时，你给不出 ARN → **真鸡生蛋**

#### 标准解法：predictable ARN（这个 repo 已经有实战范例）

[terraform/main.tf:155-157](terraform/main.tf#L155-L157) 解决 Snowflake ↔ Snowpipe 的循环依赖时用的就是这个招：
...
Snowflake 模块在自己 apply 时需要 snowpipe role 的 ARN（建 storage integration 用），但 snowpipe role 是另一个模块**之后**才创建的。解法是**约定一个稳定的命名规则，提前把 ARN 拼出来用**——不依赖 `module.snowpipe.role.arn` 这种引用，避免循环。

Cross-account dropzone 用同样的招：

1. 你和 Data.ai 提前约定 bronze role 的命名规则，比如 `iodp-dc-bronze-etl-<env>-role`
2. 把 ARN 拼出来发给 Data.ai：`arn:aws:iam::<your-account>:role/iodp-dc-bronze-etl-prod-role`
3. Data.ai 先把这个 ARN 写进他们的 bucket policy
4. 你再 `terraform apply` 创建出真实 role（**必须严格用约定的名字**，否则桶访问失败）
5. Bronze 跑起来 → 双边都通过 → 成功

**predictable ARN 把"循环依赖"打破成"单向命名约定"**，本质是用命名规约换状态依赖。这是 AWS multi-account IaC 的标准招法，也是这个项目里已经在用的设计模式。
